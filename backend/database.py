...
def get_tree_as_json(tree_id: int) -> dict | None:
    ...
    try:
        ...
        for node_row in nodes_raw:
            ...
            saved_segments = node_dict['assets'].get('segmented', [])
            if not isinstance(saved_segments, list):
                saved_segments = []

            merged_segments = []
            seen_segment_paths = set()
            for segment in saved_segments + current_node_entities:
                if not isinstance(segment, dict):
                    continue
                segment_path = segment.get('path') or segment.get('representative_thumb') or ''
                dedupe_key = segment_path or segment.get('entity_id') or json.dumps(segment, sort_keys=True)
                if dedupe_key in seen_segment_paths:
                    continue
                seen_segment_paths.add(dedupe_key)
                merged_segments.append(segment)

            node_dict['assets']['segmented'] = merged_segments
