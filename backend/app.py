...
ENTITY_OUTPUT_DIR = os.path.join(BASE_DIR, "entities")
...
def normalize_segment_paths(segments: list[dict]) -> list[dict]:
    normalized_segments = []
    for segment in segments or []:
        normalized = dict(segment)
        raw_path = str(normalized.get("path", "") or "")
        if raw_path and not raw_path.startswith(("http://", "https://", "/entities/")):
            normalized["path"] = f"/entities/{os.path.basename(raw_path)}"
        normalized_segments.append(normalized)
    return normalized_segments

def copy_remote_sam_segments_to_local(segments: list[dict]) -> None:
    if not SAM_SERVER_URL:
        return
    ...
    for segment in segments or []:
        ...
        filename = segment.get("filename") or os.path.basename(urlparse(raw_path).path)
        if not filename:
            continue

        local_path = os.path.join(ENTITY_OUTPUT_DIR, filename)
        if os.path.exists(local_path):
            continue

        remote_url = raw_path if raw_path.startswith(("http://", "https://")) else f"{SAM_SERVER_URL}/entities/{urllib.parse.quote(filename)}"
        try:
            remote_response = requests.get(remote_url, timeout=60)
            remote_response.raise_for_status()
            with open(local_path, "wb") as local_file:
                local_file.write(remote_response.content)
        except Exception as exc:
            print(f"Warning: failed to copy SAM segment {filename} from {remote_url}: {exc}")

def segment_with_sam(image_path: str, text_prompt: str, output_dir: str = ENTITY_OUTPUT_DIR) -> list[dict]:
    if SAM_SERVER_URL:
        ...
        response.raise_for_status()
        remote_segments = response.json().get("segments", [])
        copy_remote_sam_segments_to_local(remote_segments)
        return normalize_segment_paths(remote_segments)
    return normalize_segment_paths(sam_service.segment_by_text(
